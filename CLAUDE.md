...
- **プッシュは指示があるまで行わない** — ユーザーから明示的に「プッシュして」と言われるまでgit pushしない
- **【最優先】ユーザーデータの保全** — プロジェクトデータ（Supabase・localStorage）が削除・消失・上書きされる可能性がある操作を行う前に、必ずユーザーにアラートを出して確認を取る。データ損失リスクのある変更は、リスクの内容を明示してから実施する
